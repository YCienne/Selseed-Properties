...
const PricingList = () => {
  return (
    <div className=" flex gap-[9rem] max-lg:flex-wrap">
        {pricing.map((item) => (
            <div key={item.id} className="w-[19rem max-lg:w-full h-full mr-10 ml-10 px-10 bg-n-8 border border-n-3 rounded-[2rem] lg:w-auto even:py-14 odd:py-10 odd:my-10 [&>h4]:odd:text-color-2 [&>h4]:even:text-color-1">
                ...
                <Button className="w-full mb-6" href={item.price ? '/pricing' : 'mailto:contact@ignition140.com'} white={!!item.price}>
                    {item.price ? "Buy package" : "Contact Us"}
                </Button>
